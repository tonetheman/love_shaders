...
vec4 effect( vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords ) {
  ...
  vec3 v=cross(vpn,u);
  vec3 scrCoord=prp+vpn*vpd+vPos.x*u*love_ScreenSize.x/love_ScreenSize.y+vPos.y*v;
  ...
  vec4 pt=vec4(
    ...
    1.0);
  pt=pt*cm;
  vec2 xy=(pt/(-pt.z/vpd)).xy+vPos*vec2(love_ScreenSize.x/love_ScreenSize.y,1.0);
  float c;
  c=0.4/length(xy);
  pc+=vec4(
